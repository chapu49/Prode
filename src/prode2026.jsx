...
let FDB = null;
try { FDB = getDatabase(initializeApp(FIREBASE_CONFIG)); } catch(e) { console.warn("Firebase:", e); }
...
const Flag = ({name, h=18}) => {
  ...
  if (!f) return <span style={{display:'inline-block',width:w,height:h,background:'#333',borderRadius:2,verticalAlign:'middle'}}/>;
  return (
    <span style={{
...
const Team = ({name,size=13,reverse=false})=>(
  <span style={{display:'inline-flex',alignItems:'center',gap:6,flexDirection:reverse?'row-reverse':'row'}}>
    <Flag name={name} h={size+4}/>
    <span style={{fontSize:size,fontFamily:'DM Sans',fontWeight:500,color:'#e8f0ff'}}>{name}</span>
  </span>
);
...
function calcStandings(groupId, mr) {
  ...
  MATCHES.filter(m=>m.g===groupId&&m.round<=3).forEach(m=>{
    ...
    stats[m.h].gf+=h;stats[m.h].ga+=a;stats[m.h].played++;
    stats[m.a].gf+=a;stats[m.a].ga+=h;stats[m.a].played++;
...
function getBestThirdFromGroups(allowedGroups, usedThirds, mr, grpRes) {
  ...
  allowedGroups.forEach(gid=>{
    ...
    if(grpRes?.groups?.[gid]?.first) {
      ...
      if(standings.length >= 3) team3 = standings[2].team;
    } else {
      const standings = calcStandings(gid, mr);
      if(standings.length >= 3) team3 = standings[2].team;
    }
...
function autoPopulateKnockout(grpRes, mr) {
  const get1 = (g) => {
    if(grpRes?.groups?.[g]?.first) return grpRes.groups[g].first;
    const s = calcStandings(g, mr); return s[0]?.team || '';
  };
  const get2 = (g) => {
    if(grpRes?.groups?.[g]?.second) return grpRes.groups[g].second;
    const s = calcStandings(g, mr); return s[1]?.team || '';
  };
  ...
  thirdSlots.forEach(({slot,groups})=>{
    ...
    if(picked) usedThirds.push(picked);
  });

  const resolveSlot = (slot) => {
    if(!slot) return '';
    if(slot.startsWith('1')) return get1(slot[1]);
    if(slot.startsWith('2')) return get2(slot[1]);
    if(slot.startsWith('3')) return assignedThirds[slot] || '';
    return '';
  };
  ...
  MATCHES.filter(m=>m.round===4&&m.slot1).forEach(m=>{
    ...
    if(h||a) updates[m.id]={...((DB['p26_mr']||{})[m.id]||{}),teamH:h,teamA:a};
  });
  return updates;
}
...
const persist = async (k, v) => {
  DB[k] = v;
  if (FDB) {
    try { await set(ref(FDB, fbKey(k)), v); return; } catch(e) { console.warn("Firebase set:", e); }
  }
...
const tryLoad = async (k) => {
  if (FDB) {
    try {
      ...
      if (snap.exists()) { DB[k] = snap.val(); return; }
    } catch(e) { console.warn("Firebase get:", e); }
  }
  try { const r = localStorage.getItem(k); if (r) DB[k] = JSON.parse(r); } catch {}
};
...
const subscribeRT = (keys, onChange) => {
  ...
  const unsubs = keys.map(k =>
    onValue(ref(FDB, fbKey(k)), snap => {
      if (snap.exists()) { DB[k] = snap.val(); onChange(k, snap.val()); }
    })
  );
...
const countdownLabel=()=>{const diff=LOCK_DT-getNow();if(diff<=0)return null;const d=Math.floor(diff/86400000),h=Math.floor((diff%86400000)/3600000),m=Math.floor((diff%3600000)/60000);return`${d}d ${h}h ${m}m`;};
...
const matchStatus=(m,adminLocked,adminPlayed,mr={})=>{
  ...
  if(m.tbd&&!m.h&&!mr[m.id]?.teamH)return'upcoming';
  if(adminPlayed)return'played';
  const now=getNow(),lock=new Date(m.kickoff.getTime()-30*60*1000),open=new Date(m.kickoff);
  open.setUTCHours(3,0,0,0);if(open>m.kickoff)open.setUTCDate(open.getUTCDate()-1);
  if(adminLocked||now>=lock)return'locked';
...
function calcScore(pred,mr,gr,fr,appState){
  let p=0,exactos=0;
  for(const[mid,res]of Object.entries(mr||{})){if(!res.played)continue;const u=pred?.matches?.[mid];if(u?.h==null||u?.a==null)continue;const rh=+res.h,ra=+res.a;if(u.h===rh&&u.a===ra){p+=5;exactos++;continue;}if(Math.sign(u.h-u.a)===Math.sign(rh-ra))p+=3;}
  if(appState?.groupsFinished){for(const[gId,g]of Object.entries(gr?.groups||{})){const u=pred?.initPicks?.qualifiers?.[gId];if(!u||!g?.first)continue;const hf=u.first===g.first,hs=u.second===g.second;if(hf&&hs)p+=5;else if(hf)p+=3;else if(hs)p+=1;}for(const t of(pred?.initPicks?.thirds||[])){if((gr?.thirds||[]).includes(t))p+=1;}}
  if(appState?.tournamentFinished){const pod=fr?.podium;if(pod){const u=pred?.initPicks?.podium||{};if(u.p1===pod.p1)p+=10;if(u.p2===pod.p2)p+=5;if(u.p3===pod.p3)p+=3;if(u.p4===pod.p4)p+=1;}const aw=fr?.awards;if(aw){for(const{id,pts}of AWARDS){const norm=s=>s.normalize('NFD').replace(/[̀-ͯ]/g,'').trim().toLowerCase();const ua=norm(pred?.initPicks?.awards?.[id]||''),ra=norm(aw[id]||'');if(ua&&ra&&ua===ra)p+=pts;}}}
...
function useNotifications(user, mr, pred, appState) {
  ...
  useEffect(() => {
    ...
    const sinPred = MATCHES.filter(m => {
      if (m.round >= 4) return false; // solo grupos por ahora
    ...
    if (urgentes.length > 0) {
      ...
      dismiss('urgent_picks'); dismiss('pending_picks');
    }
  }, [mr, pred, user]);
  ...
  useEffect(() => {
    ...
    if (!locked && !pred.initLocked) {
      ...
      if (!hasPodium) addAlert('init_picks', 'gold', 'Cargá tus predicciones de Podio, Clasificados y Premios antes del 11 Jun', '📋');
      else dismiss('init_picks');
    } else {
      dismiss('init_picks');
    }
  }, [pred, user]);
  ...
  useEffect(() => {
    if (!user || !pred || !mr) return;
    const nuevosExactos = MATCHES.filter(m => {
      const res = mr[m.id];
      const prev = prevMr.current[m.id];
      const u = pred?.matches?.[m.id];
      if (!res?.played || prev?.played) return false; // ya estaba jugado antes
      return u?.h != null && u?.h === +res.h && u?.a === +res.a;
    });
    if (nuevosExactos.length > 0) {
      const pts = nuevosExactos.length * 5;
      addAlert(`exactos_${Date.now()}`, 'green',
        `🎯 ¡Acertaste ${nuevosExactos.length} resultado${nuevosExactos.length>1?'s':''} exacto${nuevosExactos.length>1?'s':''}! +${pts} pts`,
        '🏆'
      );
      sendBrowserNotif('¡Resultado exacto! 🎯',
        `Acertaste ${nuevosExactos.length} resultado${nuevosExactos.length>1?'s':''} y sumaste +${pts} pts`
      );
    }
    prevMr.current = {...mr};
  }, [mr]);
...
function NotificationBar({alerts, dismiss, browserPerm, requestPerm}) {
  if (!alerts.length && browserPerm !== 'default') return null;
  const colors = {
  ...
  return (
    <div style={{marginBottom:12}}>
      {browserPerm==='default'&&(
        <div style={{display:'flex',alignItems:'center',justifyContent:'space-between',padding:'8px 12px',borderRadius:10,background:'rgba(116,172,223,0.08)',border:'1px solid rgba(116,172,223,0.2)',marginBottom:6}}>
          <span style={{fontSize:12,color:'rgba(255,255,255,0.6)'}}>🔔 Activar notificaciones del navegador</span>
          <button onClick={requestPerm} style={{...st.btn('g'),padding:'4px 10px',fontSize:11,border:'1px solid rgba(116,172,223,0.3)'}}>Activar</button>
        </div>
      )}
      {alerts.map(a=>{
        const c=colors[a.type]||colors.gold;
        return(
          <div key={a.id} style={{display:'flex',alignItems:'center',justifyContent:'space-between',padding:'8px 12px',borderRadius:10,background:c.bg,border:`1px solid ${c.border}`,marginBottom:6,gap:8}}>
            <span style={{fontSize:12,color:c.text,flex:1}}>{a.msg}</span>
            <button onClick={()=>dismiss(a.id)} style={{background:'none',border:'none',cursor:'pointer',color:c.text,fontSize:14,padding:0,opacity:0.7,flexShrink:0}}>✕</button>
          </div>
        );
      })}
    </div>
...
export default function App(){
  ...
  const savePred=(np)=>{setPred(np);persist(`p26_p_${user}`,np);if(group)setAllPreds(prev=>({...prev,[user]:np}));};
  const reloadAllPreds=()=>{const preds={};Promise.all((group?.members||[]).map(m=>tryLoad(`p26_p_${m}`).then(()=>{if(DB[`p26_p_${m}`])preds[m]=DB[`p26_p_${m}`];}))).then(()=>setAllPreds({...preds}));};

  const base={fontFamily:'DM Sans, sans-serif',color:T.textPri,background:T.bg,minHeight:'100vh'};

  if(!ready)return(<div style={{...base,display:'flex',flexDirection:'column',alignItems:'center',justifyContent:'center',gap:12}}><style>{FONTS}</style><div style={{fontSize:48}}>⚽</div><p style={{color:T.textSec,fontSize:14}}>Cargando...</p></div>);

  return(
    <div style={{...base,maxWidth:720,margin:'0 auto',padding:'0 16px 60px'}}>
      <style>{FONTS}</style>
      {view==='auth'&&<AuthView onLogin={doLogin}/>}
      {view==='home'&&<HomeView user={user} mr={mr} gr={gr} fr={fr} appState={appState} onEnterGroup={doEnterGroup} onLogout={()=>{sessionStorage.removeItem('prode_user');setUser(null);setPred(EMPTY());setView('auth');}}/>}
      {view==='group'&&<GroupView user={user} group={group} pred={pred} savePred={savePred} mr={mr} gr={gr} fr={fr} appState={appState} allPreds={allPreds} tab={tab} setTab={setTab} onBack={doLeaveGroup} onSaveResults={(newMr,newGr,newFr,newState)=>{if(newMr)persist('p26_mr',newMr);if(newGr)persist('p26_gr',newGr);if(newFr)persist('p26_fr',newFr);if(newState)persist('p26_state',newState);refresh();}} reloadAllPreds={reloadAllPreds} setGroup={setGroup}/>}
    </div>
  );
}
...
function AuthView({onLogin}){
  ...
  const[invite,setInvite]=useState('');const[inviteGroup,setInviteGroup]=useState(null);
  ...
  const submit=()=>{
    ...
    if(mode==='login'){
      if(!users[uv]){setMsg('Usuario no encontrado');return;}
      if(users[uv].pass!==pv){setMsg('Contraseña incorrecta');return;}
    } else {
      if(users[uv]){setMsg('El usuario ya existe');return;}
      // El código de invitación es obligatorio para registrarse
      if(!inviteGroup){setMsg('Necesitás un código de invitación válido');return;}
  ...
  return(
    <div style={{position:'relative',minHeight:'100vh',display:'flex',flexDirection:'column',alignItems:'center',justifyContent:'center',overflow:'hidden',margin:'0 -16px',padding:'0 16px 40px'}}>

      {/* Fondo: estadio nocturno con colores de Argentina */}
      <div style={{position:'absolute',inset:0,background:'linear-gradient(180deg,#001529 0%,#002a4a 35%,#003d6b 55%,#1a5276 70%,#154360 85%,#0d2137 100%)',zIndex:0}}/>

      {/* Brillo de los focos del estadio */}
      <div style={{position:'absolute',inset:0,zIndex:1,overflow:'hidden'}}>
        <div style={{position:'absolute',top:'-5%',left:'15%',width:'40%',height:'50%',background:'radial-gradient(ellipse,rgba(116,172,223,0.18) 0%,transparent 70%)',filter:'blur(20px)'}}/>
        <div style={{position:'absolute',top:'-5%',right:'15%',width:'40%',height:'50%',background:'radial-gradient(ellipse,rgba(116,172,223,0.18) 0%,transparent 70%)',filter:'blur(20px)'}}/>
        <div style={{position:'absolute',top:'20%',left:'50%',transform:'translateX(-50%)',width:'60%',height:'40%',background:'radial-gradient(ellipse,rgba(255,255,255,0.04) 0%,transparent 70%)',filter:'blur(30px)'}}/>
      </div>

      {/* Silueta del estadio */}
      <svg style={{position:'absolute',bottom:0,left:0,right:0,width:'100%',zIndex:2,opacity:0.35}} viewBox="0 0 800 200" preserveAspectRatio="none">
        {/* Tribunas */}
        <path d="M0,200 L0,120 Q50,80 100,90 L150,60 L200,70 L250,40 L300,50 L350,20 L400,30 L450,20 L500,50 L550,40 L600,70 L650,60 L700,90 Q750,80 800,120 L800,200 Z" fill="#0a1929"/>
        <path d="M0,200 L0,140 Q80,110 160,120 L200,100 L250,85 L350,60 L400,50 L450,60 L550,85 L600,100 L640,120 Q720,110 800,140 L800,200 Z" fill="#0d2040"/>
        {/* Pasto */}
        <rect x="0" y="170" width="800" height="30" fill="#0f5132" opacity="0.8"/>
        <rect x="0" y="175" width="800" height="4" fill="#1a6b42" opacity="0.6"/>
        {/* Arcos */}
        <rect x="320" y="155" width="20" height="20" fill="none" stroke="#ffffff22" strokeWidth="1.5"/>
        <rect x="460" y="155" width="20" height="20" fill="none" stroke="#ffffff22" strokeWidth="1.5"/>
        {/* Luces del estadio */}
        <circle cx="80" cy="75" r="8" fill="#fff9c4" opacity="0.9"/>
        <circle cx="80" cy="75" r="20" fill="#fff9c4" opacity="0.2"/>
        <circle cx="720" cy="75" r="8" fill="#fff9c4" opacity="0.9"/>
        <circle cx="720" cy="75" r="20" fill="#fff9c4" opacity="0.2"/>
        <circle cx="200" cy="45" r="6" fill="#fff9c4" opacity="0.8"/>
        <circle cx="600" cy="45" r="6" fill="#fff9c4" opacity="0.8"/>
      </svg>

      {/* Estrellas de fondo */}
      <svg style={{position:'absolute',inset:0,width:'100%',height:'100%',zIndex:1}} viewBox="0 0 100 100" preserveAspectRatio="none">
        {stars.map(s=><circle key={s.id} cx={s.x} cy={s.y} r={s.size*0.3} fill="#fff" opacity={s.opacity*0.6}/>)}
      </svg>
      ...
      <svg style={{position:'absolute',inset:0,width:'100%',height:'100%',zIndex:3,pointerEvents:'none'}} viewBox="0 0 100 100" preserveAspectRatio="none">
        {confetti.map(c=>(
          <rect key={c.id} x={c.x} y={c.y} width={c.size*0.5} height={c.size*0.2}
            fill={c.color} opacity={0.7} transform={`rotate(${c.rot},${c.x},${c.y})`}/>
        ))}
      </svg>

      {/* Cinta — colores de los países anfitriones */}
      <div style={{position:'absolute',top:0,left:0,right:0,height:5,background:'linear-gradient(to right,#b22234,#fff,#006847,#fff,#ff0000,#fff,#b22234)',zIndex:4,opacity:0.7}}/>

      {/* Contenido */}
      <div style={{position:'relative',zIndex:10,width:'100%',maxWidth:400,padding:'0 0 20px'}}>

        {/* Hero */}
        <div style={{textAlign:'center',marginBottom:28}}>

          <div style={{fontSize:52,marginBottom:8,filter:'drop-shadow(0 0 18px rgba(251,191,36,0.6))'}}>🏆</div>

          <div style={{position:'relative',marginBottom:4}}>
            <div style={{fontFamily:'Bebas Neue',fontSize:58,lineHeight:0.92,letterSpacing:6,background:'linear-gradient(180deg,#ffffff 0%,#c8dff7 60%,#74acdf 100%)',WebkitBackgroundClip:'text',WebkitTextFillColor:'transparent',backgroundClip:'text',filter:'drop-shadow(0 2px 12px rgba(116,172,223,0.5))'}}>PRODE</div>
            <div style={{fontFamily:'Bebas Neue',fontSize:46,lineHeight:1,letterSpacing:8,background:'linear-gradient(180deg,#ffffff 0%,#e0eefa 100%)',WebkitBackgroundClip:'text',WebkitTextFillColor:'transparent',backgroundClip:'text'}}>MUNDIAL</div>
            <div style={{fontFamily:'Bebas Neue',fontSize:86,lineHeight:0.9,letterSpacing:4,background:`linear-gradient(180deg,${T.green} 0%,#00a855 60%,#007a3d 100%)`,WebkitBackgroundClip:'text',WebkitTextFillColor:'transparent',backgroundClip:'text',filter:`drop-shadow(0 0 30px ${T.green}88)`}}>2026</div>
          </div>

          <div style={{display:'flex',justifyContent:'center',alignItems:'center',gap:20,marginTop:14,marginBottom:6}}>
            {[['EE.UU.','us'],['México','mx'],['Canadá','ca']].map(([name,code])=>(
              <div key={code} style={{display:'flex',flexDirection:'column',alignItems:'center',gap:5}}>
                <Flag name={name} h={24}/>
                <span style={{fontSize:11,color:'rgba(255,255,255,0.7)',letterSpacing:'0.05em',fontWeight:500}}>{name}</span>
              </div>
            ))}
          </div>

          <div style={{marginTop:10,display:'inline-block',padding:'4px 16px',borderRadius:20,border:'1px solid rgba(255,255,255,0.15)',background:'rgba(255,255,255,0.06)'}}>
            <span style={{fontSize:12,color:'rgba(255,255,255,0.55)',letterSpacing:'0.08em'}}>11 JUN — 19 JUL 2026</span>
          </div>

        </div>

        {/* Card */}
        <div style={{background:'rgba(8,13,23,0.85)',backdropFilter:'blur(20px)',border:'1px solid rgba(116,172,223,0.2)',borderRadius:16,padding:'24px 24px 20px',boxShadow:'0 20px 60px rgba(0,0,0,0.6)'}}>

          {mode==='login'&&(<>
            <div style={{fontFamily:'Bebas Neue',fontSize:18,letterSpacing:3,color:T.textPri,marginBottom:20,textAlign:'center'}}>INICIAR SESIÓN</div>
            <label style={{fontSize:11,fontWeight:600,letterSpacing:'0.08em',color:'rgba(255,255,255,0.4)',display:'block',marginBottom:6,textTransform:'uppercase'}}>Usuario</label>
            <input style={{...st.inp,background:'rgba(255,255,255,0.06)',border:'1px solid rgba(116,172,223,0.25)',color:'#fff'}} value={u} onChange={e=>setU(e.target.value)} placeholder="tu_usuario" onKeyDown={e=>e.key==='Enter'&&submit()} autoFocus/>
            <label style={{fontSize:11,fontWeight:600,letterSpacing:'0.08em',color:'rgba(255,255,255,0.4)',display:'block',marginBottom:6,marginTop:4,textTransform:'uppercase'}}>Contraseña</label>
            <input style={{...st.inp,background:'rgba(255,255,255,0.06)',border:'1px solid rgba(116,172,223,0.25)',color:'#fff'}} type="password" value={p} onChange={e=>setP(e.target.value)} placeholder="••••••••" onKeyDown={e=>e.key==='Enter'&&submit()}/>
            {msg&&<p style={{fontSize:12,color:'#f87171',marginTop:4,marginBottom:8}}>{msg}</p>}
            <button style={{...st.btn('p'),width:'100%',padding:'11px',marginTop:8,fontSize:14,fontWeight:600,letterSpacing:'0.04em',boxShadow:`0 0 20px ${T.green}44`}} onClick={submit}>
              Ingresar
            </button>
            <div style={{textAlign:'center',marginTop:16}}>
              <button onClick={()=>{setMode('register');setMsg('');setU('');setP('');}} style={{background:'none',border:'none',cursor:'pointer',fontSize:12,color:'rgba(116,172,223,0.7)',fontFamily:'DM Sans',textDecoration:'underline',textUnderlineOffset:3}}>
          ...
          {mode==='register'&&(<>
            <div style={{display:'flex',alignItems:'center',gap:10,marginBottom:20}}>
              <button onClick={()=>{setMode('login');setMsg('');setInvite('');setInviteGroup(null);}} style={{background:'none',border:'none',cursor:'pointer',color:'rgba(116,172,223,0.6)',fontSize:18,padding:0,lineHeight:1}}>←</button>
              <div style={{fontFamily:'Bebas Neue',fontSize:18,letterSpacing:3,color:T.textPri}}>CREAR CUENTA</div>
            </div>
            <label style={{fontSize:11,fontWeight:600,letterSpacing:'0.08em',color:'rgba(255,255,255,0.4)',display:'block',marginBottom:6,textTransform:'uppercase'}}>Código de invitación</label>
            <input style={{...st.inp,background:'rgba(255,255,255,0.06)',border:`1px solid ${inviteGroup?T.green:'rgba(116,172,223,0.25)'}`,color:'#fff',textTransform:'uppercase',letterSpacing:6,fontFamily:'Bebas Neue',fontSize:18,textAlign:'center'}} value={invite} onChange={e=>setInvite(e.target.value.toUpperCase().slice(0,7))} placeholder="XXXXXXX" autoFocus/>
            {inviteGroup&&<div style={{fontSize:12,color:T.green,marginBottom:8,padding:'6px 12px',background:T.greenDim,borderRadius:8,border:`1px solid ${T.green}33`,display:'flex',alignItems:'center',gap:6}}>✓ Grupo: <strong style={{textTransform:'capitalize'}}>{inviteGroup}</strong></div>}
            {invite.length===7&&!inviteGroup&&<p style={{fontSize:12,color:'#f87171',marginBottom:8}}>Código inválido</p>}
            {inviteGroup&&(<>
              <label style={{fontSize:11,fontWeight:600,letterSpacing:'0.08em',color:'rgba(255,255,255,0.4)',display:'block',marginBottom:6,marginTop:8,textTransform:'uppercase'}}>Elegí un usuario</label>
              <input style={{...st.inp,background:'rgba(255,255,255,0.06)',border:'1px solid rgba(116,172,223,0.25)',color:'#fff'}} value={u} onChange={e=>setU(e.target.value)} placeholder="tu_usuario" onKeyDown={e=>e.key==='Enter'&&submit()}/>
              <label style={{fontSize:11,fontWeight:600,letterSpacing:'0.08em',color:'rgba(255,255,255,0.4)',display:'block',marginBottom:6,marginTop:4,textTransform:'uppercase'}}>Contraseña</label>
              <input style={{...st.inp,background:'rgba(255,255,255,0.06)',border:'1px solid rgba(116,172,223,0.25)',color:'#fff'}} type="password" value={p} onChange={e=>setP(e.target.value)} placeholder="••••••••" onKeyDown={e=>e.key==='Enter'&&submit()}/>
            </>)}
            {msg&&msg!=='Código de invitación inválido'&&<p style={{fontSize:12,color:'#f87171',marginTop:4,marginBottom:8}}>{msg}</p>}
            {inviteGroup&&(
              <button style={{...st.btn('p'),width:'100%',padding:'11px',marginTop:8,fontSize:14,fontWeight:600,letterSpacing:'0.04em',boxShadow:`0 0 20px ${T.green}44`}} onClick={submit}>
                Registrarse y entrar al prode
              </button>
            )}
          </>)}

        </div>
        <p style={{textAlign:'center',fontSize:11,color:'rgba(255,255,255,0.25)',marginTop:14}}>Versión de prueba · datos compartidos entre sesiones</p>
      </div>
    </div>
  );
}
...
function HomeView({user,mr,gr,fr,appState,onEnterGroup,onLogout}){
  ...
  const[gName,setGName]=useState('');const[gPass,setGPass]=useState('');const[gMsg,setGMsg]=useState('');
  const[globalRows,setGlobalRows]=useState([]);const[loadingG,setLoadingG]=useState(false);
  const allGroups=DB['p26_groups']||{};
  const myGroups=Object.entries(allGroups).filter(([,g])=>g.members.includes(user)).map(([name,g])=>({name,...g}));

  const handleGroup=()=>{const name=gName.trim(),pass=gPass.trim();if(!name||!pass){setGMsg('Completá los dos campos');return;}const groups=DB['p26_groups']||{};const key=name.toLowerCase();if(showForm==='create'){if(groups[key]){setGMsg('Ya existe un grupo con ese nombre');return;}groups[key]={password:pass,admin:user,members:[user],created:Date.now()};}else{if(!groups[key]){setGMsg('Grupo no encontrado');return;}if(groups[key].password!==pass){setGMsg('Contraseña incorrecta');return;}if(!groups[key].members.includes(user))groups[key].members.push(user);}persist('p26_groups',groups);setShowForm(null);setGName('');setGPass('');setGMsg('');onEnterGroup({name:key,...groups[key]});};
  ...
  useEffect(()=>{if(htab==='global')loadGlobal();},[htab]);
    ...
    <div style={{paddingTop:16}}>
      ...
      <div style={{display:'flex',justifyContent:'space-between',alignItems:'center',paddingBottom:16,borderBottom:`1px solid ${T.border}`,marginBottom:16}}>
        <div style={{display:'flex',alignItems:'center',gap:10}}>
          <span style={{fontFamily:'Bebas Neue',fontSize:20,letterSpacing:2,color:T.green}}>PRODE MUNDIAL</span>
          <span style={{fontFamily:'Bebas Neue',fontSize:20,letterSpacing:2,color:T.textPri}}>2026</span>
        </div>
        <div style={{display:'flex',alignItems:'center',gap:8}}>
          <div style={{width:28,height:28,borderRadius:8,background:T.greenDim,border:`1px solid ${T.green}44`,display:'flex',alignItems:'center',justifyContent:'center',fontSize:12,color:T.green,fontWeight:600}}>
            {user?.[0]?.toUpperCase()}
          </div>
          <span style={{fontSize:12,color:T.textMid}}>{user}</span>
          <button onClick={onLogout} style={{marginLeft:4,padding:'5px 12px',border:`1px solid ${T.border}`,borderRadius:8,background:'transparent',cursor:'pointer',fontSize:11,color:T.textSec,fontFamily:'DM Sans',letterSpacing:'0.02em'}}>
            Salir
          </button>
        </div>
      </div>
      ...
      <div style={{display:'flex',borderBottom:`1px solid ${T.border}`,marginBottom:20}}>
        {[{k:'groups',l:'Mis grupos'},{k:'global',l:'Tabla general'}].map(({k,l})=><button key={k} style={st.tab(htab===k)} onClick={()=>setHtab(k)}>{l}</button>)}
      </div>
      ...
      {htab==='groups'&&(<div>
        {myGroups.length===0&&!showForm&&(
          <div style={{textAlign:'center',padding:'40px 20px',color:T.textSec}}>
            ...
            <p style={{fontSize:14}}>No estás en ningún grupo todavía</p>
          </div>
        )}
        {myGroups.map(g2=>(
        ...
        {!showForm&&(<div style={{display:'flex',gap:8,marginTop:12}}>
          <button style={{...st.btn('p'),flex:1,padding:'10px'}} onClick={()=>{setShowForm('create');setGMsg('');setGName('');setGPass('');}}>+ Crear grupo</button>
          <button style={{...st.btn('d'),flex:1,padding:'10px',border:`1px solid ${T.border}`}} onClick={()=>{setShowForm('join');setGMsg('');setGName('');setGPass('');}}>Unirse a grupo</button>
        </div>)}
        {showForm&&(<div style={{...st.card,marginTop:12,borderColor:T.green+'44'}}>
          <h3 style={{fontSize:14,fontWeight:600,marginBottom:16,color:T.textPri}}>{showForm==='create'?'Crear nuevo grupo':'Unirse a un grupo'}</h3>
          <label style={{fontSize:11,fontWeight:600,letterSpacing:'0.08em',color:T.textSec,display:'block',marginBottom:6,textTransform:'uppercase'}}>Nombre del grupo</label>
          <input style={st.inp} value={gName} onChange={e=>{setGName(e.target.value);setGMsg('');}} placeholder="Ej: Los Pibes" autoFocus onKeyDown={e=>e.key==='Enter'&&handleGroup()}/>
          <label style={{fontSize:11,fontWeight:600,letterSpacing:'0.08em',color:T.textSec,display:'block',marginBottom:6,marginTop:4,textTransform:'uppercase'}}>Contraseña del grupo</label>
          <input style={st.inp} type="password" value={gPass} onChange={e=>{setGPass(e.target.value);setGMsg('');}} placeholder="Contraseña de acceso" onKeyDown={e=>e.key==='Enter'&&handleGroup()}/>
          {gMsg&&<p style={{fontSize:12,color:T.red,marginTop:4,marginBottom:8}}>{gMsg}</p>}
          <div style={{display:'flex',gap:8,marginTop:4}}>
            <button style={st.btn('p')} onClick={handleGroup}>{showForm==='create'?'Crear y entrar':'Unirse y entrar'}</button>
            <button style={{...st.btn(),border:`1px solid ${T.border}`}} onClick={()=>{setShowForm(null);setGMsg('');}}>Cancelar</button>
          </div>
        </div>)}
      </div>)}
      ...
      {htab==='global'&&(<div>
        <div style={{display:'flex',justifyContent:'space-between',alignItems:'center',marginBottom:16}}>
          <p style={{fontSize:13,color:T.textSec}}>Todos los participantes del prode</p>
          <button style={{...st.btn(),fontSize:12,padding:'6px 12px',border:`1px solid ${T.border}`}} onClick={loadGlobal} disabled={loadingG}>{loadingG?'...':'↻ Actualizar'}</button>
        </div>
        <Leaderboard data={globalRows} currentUser={user}/>
      </div>)}
    </div>
  );
}
...
function GroupView({user,group,pred,savePred,mr,gr,fr,appState,allPreds,tab,setTab,onBack,onSaveResults,reloadAllPreds,setGroup}){
  ...
  return(
    <div style={{paddingTop:16}}>
      {/* Header grupo */}
      {appState?.simDate&&<div style={{background:'rgba(232,93,38,0.15)',border:`1px solid ${T.gold}44`,borderRadius:8,padding:'6px 12px',marginBottom:10,fontSize:11,color:T.gold,display:'flex',alignItems:'center',gap:6}}>⚠ Modo simulación activo · {new Date(appState.simDate).toLocaleString('es-AR',{timeZone:'America/Argentina/Buenos_Aires',dateStyle:'short',timeStyle:'short'})}</div>}
      <NotificationBar alerts={alerts} dismiss={dismiss} browserPerm={browserPerm} requestPerm={requestPerm}/>
      <div style={{display:'flex',alignItems:'center',gap:12,marginBottom:16,paddingBottom:16,borderBottom:`1px solid ${T.border}`}}>
        <button onClick={onBack} style={{...st.btn(),padding:'8px 12px',fontSize:12,border:`1px solid ${T.border}`}}>← Salir</button>
        <div>
          <div style={{fontFamily:'Bebas Neue',fontSize:20,letterSpacing:2,color:T.textPri,textTransform:'uppercase'}}>{group.name}</div>
          <div style={{fontSize:11,color:T.textSec}}>{group.members.length} participantes{isAdmin&&' · Admin'}</div>
        </div>
      </div>
      {/* Tabs */}
      <div style={{display:'flex',borderBottom:`1px solid ${T.border}`,marginBottom:20,overflowX:'auto'}}>
        {tabs.map(t=><button key={t.k} style={st.tab(tab===t.k)} onClick={()=>setTab(t.k)}>{t.l}</button>)}
      </div>
      {tab==='picks'   &&<PicksTab   pred={pred} savePred={savePred} appState={appState}/>}
      {tab==='matches' &&<MatchesTab pred={pred} savePred={savePred} mr={mr} allPreds={allPreds} user={user}/>}
      {tab==='table'   &&<TableTab   allPreds={allPreds} mr={mr} gr={gr} fr={fr} appState={appState} currentUser={user} reloadAllPreds={reloadAllPreds}/>}
      {tab==='admin'   &&<AdminTab   mr={mr} gr={gr} fr={fr} appState={appState} onSave={onSaveResults} group={group} setGroup={setGroup} user={user}/>}
    </div>
  );
}
...
function PicksTab({pred,savePred,appState}){
  ...
  const clickQ=(gId,t)=>{if(timeLocked)return;const q={...(local.qualifiers?.[gId]||{})};if(q.first===t){q.first=q.second;q.second=null;}else if(q.second===t){q.second=null;}else if(!q.first)q.first=t;else if(!q.second)q.second=t;else q.second=t;saveLocal({...local,qualifiers:{...local.qualifiers,[gId]:q}});};
  const togT=(t)=>{if(timeLocked)return;const cur=local.thirds||[];saveLocal({...local,thirds:cur.includes(t)?cur.filter(x=>x!==t):cur.length<8?[...cur,t]:cur});};
  const setPodium=(k,v)=>{if(timeLocked)return;saveLocal({...local,podium:{...local.podium,[k]:v}});};
  ...
  return(<div>
    ...
    <div style={{display:'flex',flexWrap:'wrap',gap:6,marginBottom:16}}>
      {!timeLocked&&countdown&&<span style={st.chip('gold')}>⏳ Cierre en {countdown}</span>}
      {timeLocked&&<span style={st.chip('red')}>🔒 Predicciones cerradas</span>}
      {saved&&<span style={st.chip('green')}>✓ Guardado</span>}
      {appState.groupsFinished&&<span style={st.chip('green')}>✓ Grupos finalizados</span>}
      {areAwardsOpen()&&!appState.tournamentFinished&&<span style={st.chip('gold')}>🏅 Premios FIFA habilitados</span>}
      {appState.tournamentFinished&&<span style={st.chip('green')}>✓ Torneo finalizado</span>}
    </div>
    {/* Section nav */}
    <div style={{display:'flex',gap:6,marginBottom:20,flexWrap:'wrap'}}>
      {secs.map(s=><button key={s.k} onClick={()=>setSec(s.k)} style={{padding:'7px 16px',border:`1px solid ${sec===s.k?T.green:T.border}`,borderRadius:20,fontSize:12,cursor:'pointer',background:sec===s.k?T.greenDim:'transparent',color:sec===s.k?T.green:T.textSec,fontFamily:'DM Sans',fontWeight:sec===s.k?600:400,transition:'all 0.15s'}}>{s.l}</button>)}
    </div>

    {/* PODIO */}
    {sec==='podium'&&(<div>
      <p style={{fontSize:13,color:T.textSec,marginBottom:16}}>¿Quiénes quedarán en los 4 primeros puestos? Se guarda automáticamente.</p>
      <div style={{display:'grid',gridTemplateColumns:'repeat(auto-fill,minmax(210px,1fr))',gap:10}}>
        {[{k:'p1',icon:'🥇',l:'Campeón',pts:10},{k:'p2',icon:'🥈',l:'Subcampeón',pts:5},{k:'p3',icon:'🥉',l:'3° Puesto',pts:3},{k:'p4',icon:'4°',l:'4° Puesto',pts:1}].map(({k,icon,l,pts})=>(
          <div key={k} style={{...st.card,borderTop:`2px solid ${pts===10?T.gold:pts===5?T.textSec:pts===3?'#cd7f32':'transparent'}`}}>
            <div style={{display:'flex',justifyContent:'space-between',alignItems:'flex-start',marginBottom:10}}>
              <div>
                <div style={{fontFamily:'Bebas Neue',fontSize:22,letterSpacing:1,color:T.textPri}}>{icon} {l}</div>
              </div>
              <span style={{...st.chip('gold'),fontSize:10}}>{pts} pts</span>
            </div>
            <select disabled={timeLocked} value={local.podium?.[k]||''} onChange={e=>setPodium(k,e.target.value)} style={{...st.inp,marginBottom:0,cursor:timeLocked?'default':'pointer',opacity:timeLocked?0.5:1}}>
              <option value=''>Elegir selección...</option>
              {ALL_TEAMS.map(t=><option key={t} value={t}>{t}</option>)}
            </select>
            {local.podium?.[k]&&<div style={{marginTop:8,display:'flex',alignItems:'center',gap:6}}><Flag name={local.podium[k]} h={16}/><span style={{fontSize:13,color:T.green,fontWeight:500}}>{local.podium[k]}</span></div>}
          </div>
        ))}
      </div>
    </div>)}
    ...
    {/* CLASIFICADOS */}
    {sec==='qualifiers'&&(<div>
      <p style={{fontSize:13,color:T.textSec,marginBottom:16}}>Clic = 1°, clic en otro = 2°. Clic en el elegido para quitar.</p>
      <div style={{display:'grid',gridTemplateColumns:'repeat(auto-fill,minmax(185px,1fr))',gap:8}}>
        {GRP.map(gg=>{const q=local.qualifiers?.[gg.id]||{};return(<div key={gg.id} style={{...st.card,padding:0,overflow:'hidden',margin:0}}>
          <div style={{padding:'8px 12px',borderBottom:`1px solid ${T.border}`,background:T.bgCardHi,display:'flex',justifyContent:'space-between',alignItems:'center'}}>
            <span style={{fontFamily:'Bebas Neue',fontSize:15,letterSpacing:2,color:T.green}}>GRUPO {gg.id}</span>
            <div style={{display:'flex',gap:4}}>
              {q.first&&<span style={{...st.chip('gold'),fontSize:9,padding:'1px 6px'}}>1° {q.first.split(' ')[0]}</span>}
              {q.second&&<span style={{...st.chip(''),fontSize:9,padding:'1px 6px'}}>2° {q.second.split(' ')[0]}</span>}
            </div>
          </div>
          {gg.t.map(t=>{const iF=q.first===t,iS=q.second===t;return(<button key={t} onClick={()=>clickQ(gg.id,t)} style={{display:'flex',alignItems:'center',gap:8,width:'100%',padding:'9px 12px',border:'none',borderTop:`1px solid ${T.border}`,cursor:timeLocked?'default':'pointer',background:iF?'rgba(251,191,36,0.1)':iS?'rgba(59,130,246,0.1)':'transparent',transition:'background 0.1s'}}>
            <Flag name={t} h={15}/>
            <span style={{flex:1,fontSize:12.5,color:iF?T.gold:iS?T.blue:T.textPri,fontWeight:iF||iS?600:400,textAlign:'left'}}>{t}</span>
            {iF&&<span style={{fontSize:10,fontWeight:700,color:T.gold}}>1°</span>}
            {iS&&<span style={{fontSize:10,fontWeight:700,color:T.blue}}>2°</span>}
          </button>);})}
        </div>);})}
      </div>
    </div>)}
    ...
    {sec==='thirds'&&(<div>
      <div style={{display:'flex',justifyContent:'space-between',alignItems:'center',marginBottom:16}}>
        <p style={{fontSize:13,color:T.textSec}}>Elegí 8 equipos que clasificarán como mejores terceros</p>
        <span style={{...st.chip((local.thirds||[]).length===8?'green':'gold'),fontSize:12}}>{(local.thirds||[]).length}/8</span>
      </div>
      <div style={{display:'grid',gridTemplateColumns:'repeat(auto-fill,minmax(185px,1fr))',gap:8}}>
        {t3.map(gg=>(<div key={gg.id} style={{...st.card,padding:0,overflow:'hidden',margin:0}}>
          <div style={{padding:'8px 12px',borderBottom:`1px solid ${T.border}`,background:T.bgCardHi}}>
            <span style={{fontFamily:'Bebas Neue',fontSize:15,letterSpacing:2,color:'#a855f7'}}>GRUPO {gg.id}</span>
          </div>
          {gg.teams.map(t=>{const sel=(local.thirds||[]).includes(t),dis=!sel&&(local.thirds||[]).length>=8;return(<button key={t} onClick={()=>togT(t)} disabled={timeLocked||dis} style={{display:'flex',alignItems:'center',gap:8,width:'100%',padding:'9px 12px',border:'none',borderTop:`1px solid ${T.border}`,cursor:(timeLocked||dis)?'default':'pointer',background:sel?'rgba(168,85,247,0.12)':'transparent',opacity:dis?0.3:1,transition:'background 0.1s'}}>
            <Flag name={t} h={15}/>
            <span style={{flex:1,fontSize:12.5,color:sel?'#c084fc':T.textPri,fontWeight:sel?600:400,textAlign:'left'}}>{t}</span>
            {sel&&<span style={{color:'#a855f7',fontSize:14}}>✓</span>}
          </button>);})}
        </div>))}
      </div>
    </div>)}
    ...
    {/* PREMIOS FIFA */}
    {sec==='awards'&&(<div>
      {!appState.groupsFinished&&(<div style={{...st.card,textAlign:'center',padding:'40px 20px',borderStyle:'dashed'}}>
        <div style={{fontSize:36,marginBottom:12,opacity:0.5}}>🏅</div>
        <p style={{fontWeight:600,color:T.textPri,marginBottom:6}}>Premios FIFA</p>
        <p style={{color:T.textSec,fontSize:13}}>Se habilitarán cuando finalice la fase de grupos</p>
      </div>)}
      {appState.groupsFinished&&(<div>
        <p style={{fontSize:13,color:T.textSec,marginBottom:16}}>
        {awardsLocked?'Torneo finalizado — premios bloqueados':'Elegí de la lista oficial de candidatos FIFA · Podés cambiar hasta que finalice el torneo'}
      </p>
        <div style={{display:'grid',gridTemplateColumns:'repeat(auto-fill,minmax(210px,1fr))',gap:10}}>
          {AWARDS.map(aw=>(<div key={aw.id} style={st.card}>
            <div style={{display:'flex',justifyContent:'space-between',alignItems:'center',marginBottom:8}}>
              <span style={{fontSize:13,fontWeight:600,color:T.textPri}}>{aw.label}</span>
              <span style={{...st.chip('gold'),fontSize:10}}>{aw.pts} pts</span>
            </div>
            <select disabled={awardsLocked} value={localAwards[aw.id]||''} onChange={e=>setLocalAwards(a=>({...a,[aw.id]:e.target.value}))} style={{...st.inp,marginBottom:0,opacity:awardsLocked?0.5:1,cursor:awardsLocked?'default':'pointer'}}>
              <option value=''>— Elegir candidato —</option>
              {(AWARD_PLAYERS[AWARD_PLAYER_LIST[aw.id]]||[]).map(p=><option key={p} value={p}>{p}</option>)}
            </select>
            {localAwards[aw.id]&&<div style={{fontSize:11,color:T.green,marginTop:4}}>✓ {localAwards[aw.id]}</div>}
          </div>))}
        </div>
        {!awardsLocked&&(<div style={{marginTop:16,display:'flex',alignItems:'center',gap:10}}>
          <button style={st.btn('p')} onClick={saveAwards} disabled={savingAw}>{savingAw?'Guardando...':'Guardar premios'}</button>
          {saved&&<span style={st.chip('green')}>✓ Guardado</span>}
        </div>)}
      </div>)}
    </div>)}

    {sec!=='awards'&&<p style={{fontSize:11,color:T.textSec,marginTop:16,opacity:0.6}}>{timeLocked?'🔒 Predicciones cerradas el 11 jun 14:30 hs (Argentina)':'Las predicciones se guardan automáticamente · Cierre: 11 jun 14:30 hs (Argentina)'}</p>}
  </div>);
}
...
function MatchesTab({pred,savePred,mr,allPreds,user}){
  ...
  const saveMatch=(mid)=>{const m=local[mid];if(m?.h==null||m?.a==null)return;setSaving(mid);savePred({...pred,matches:{...pred.matches,[mid]:m}});setSaving(null);};
  ...
  return(<div>
    <p style={{fontSize:13,color:T.textSec,marginBottom:20}}>Cada partido se habilita a medianoche del día que se juega (hora Argentina)</p>
    {dateOrder.map(date=>{
      const dayMs=byDate[date];
      return(<div key={date} style={{marginBottom:28}}>
        <div style={{display:'flex',alignItems:'center',gap:10,marginBottom:12}}>
          <div style={{height:1,width:12,background:T.border}}/>
          <h3 style={{fontSize:11,fontWeight:600,letterSpacing:'0.1em',textTransform:'uppercase',color:date.startsWith('rnd_')?T.gold:T.textSec,whiteSpace:'nowrap'}}>{date.startsWith('rnd_')?ROUND_LABELS[+date.slice(4)]:date}</h3>
          <div style={{height:1,flex:1,background:T.border}}/>
        </div>
        {dayMs.map(m=>{
          const status=getStatus(m);
          const sc=statusConfig[status];
          const res=mr[m.id];const p=local[m.id]||{};const sv=pred.matches?.[m.id];
          const locked=status!=='open';
          let pts=null;if(res?.played&&sv?.h!=null){pts=sv.h===res.h&&sv.a===res.a?5:Math.sign(sv.h-sv.a)===Math.sign(res.h-res.a)?3:0;}
          return(<div key={m.id} style={{...st.card,marginBottom:8,opacity:status==='upcoming'?0.55:1,borderLeft:`3px solid ${status==='open'?T.green:status==='played'?T.green:status==='locked'?T.gold:T.border}`}}>
            {/* Meta */}
            <div style={{display:'flex',justifyContent:'space-between',alignItems:'center',marginBottom:12}}>
              <span style={{fontSize:11,color:T.textSec}}>{m.round>=4?(m.label||`Ronda ${m.round}`):`Grupo ${m.g}`} · {m.timeART} hs{m.venue?` · ${m.venue}`:''}</span>
              <span style={st.chip(sc.chipV)}>{status==='played'?'✓ Jugado':status==='open'?'● Abierto':status==='locked'?'🔒 Cerrado':'⏳ Próximamente'}</span>
            </div>
            {/* Score */}
            <div style={{display:'grid',gridTemplateColumns:'1fr auto 1fr',alignItems:'center',gap:12}}>
              <div style={{textAlign:'right',display:'flex',flexDirection:'column',alignItems:'flex-end',gap:4}}>
                <Flag name={m.h} h={20}/>
                <span style={{fontSize:12,color:T.textMid,fontWeight:500}}>{m.h}</span>
              </div>
              <div style={{display:'flex',gap:8,alignItems:'center'}}>
                <input type="number" min={0} max={20} disabled={locked} value={p.h??''} onChange={e=>setLocal(prev=>({...prev,[m.id]:{...prev[m.id],h:e.target.value===''?null:+e.target.value}}))} style={{width:52,height:52,textAlign:'center',border:`1px solid ${!locked?T.green:T.border}`,borderRadius:10,background:!locked?'rgba(0,201,107,0.06)':T.bgInput,color:T.textPri,fontSize:22,fontFamily:'Bebas Neue',letterSpacing:1,cursor:locked?'default':'text',outline:'none',transition:'border-color 0.15s'}}/>
                <span style={{fontFamily:'Bebas Neue',fontSize:18,color:T.textSec,letterSpacing:2}}>VS</span>
                <input type="number" min={0} max={20} disabled={locked} value={p.a??''} onChange={e=>setLocal(prev=>({...prev,[m.id]:{...prev[m.id],a:e.target.value===''?null:+e.target.value}}))} style={{width:52,height:52,textAlign:'center',border:`1px solid ${!locked?T.green:T.border}`,borderRadius:10,background:!locked?'rgba(0,201,107,0.06)':T.bgInput,color:T.textPri,fontSize:22,fontFamily:'Bebas Neue',letterSpacing:1,cursor:locked?'default':'text',outline:'none',transition:'border-color 0.15s'}}/>
              </div>
              <div style={{textAlign:'left',display:'flex',flexDirection:'column',alignItems:'flex-start',gap:4}}>
                <Flag name={m.a} h={20}/>
                <span style={{fontSize:12,color:T.textMid,fontWeight:500}}>{m.a}</span>
              </div>
            </div>
            {/* Footer */}
            <div style={{display:'flex',justifyContent:'space-between',alignItems:'center',marginTop:12}}>
              <div style={{fontSize:12,color:T.textSec}}>
                {res?.played&&<span>Resultado: <strong style={{color:T.textPri}}>{res.h} - {res.a}</strong></span>}
                {!res?.played&&sv?.h!=null&&status!=='upcoming'&&<span style={{color:T.green}}>✓ Guardado: {sv.h}-{sv.a}</span>}
              </div>
              <div style={{display:'flex',gap:8,alignItems:'center'}}>
                {res?.played&&<button style={{...st.btn(),fontSize:11,padding:'5px 12px',border:`1px solid ${T.border}`}} onClick={()=>setShowO(showO===m.id?null:m.id)}>{showO===m.id?'Ocultar':'Ver predicciones'}</button>}
                {pts!==null&&<div style={{...st.chip(pts>0?'green':''),padding:'4px 10px'}}>{pts>0?`+${pts} pts`:'0 pts'}</div>}
                {status==='open'&&<button style={{...st.btn('p'),padding:'7px 18px',fontSize:12}} onClick={()=>saveMatch(m.id)} disabled={saving===m.id||p.h==null||p.a==null}>{saving===m.id?'...':'Guardar'}</button>}
              </div>
            </div>
            {/* Others */}
            {showO===m.id&&res?.played&&(<div style={{marginTop:12,paddingTop:12,borderTop:`1px solid ${T.border}`}}>
              {Object.entries(allPreds).map(([u2,up])=>{const mp=up.matches?.[m.id];if(!mp||mp.h==null)return null;const p2=mp.h===res.h&&mp.a===res.a?5:Math.sign(mp.h-mp.a)===Math.sign(res.h-res.a)?3:0;return(<div key={u2} style={{display:'flex',justifyContent:'space-between',alignItems:'center',fontSize:12,padding:'5px 0',borderBottom:`1px solid ${T.border}11`}}>
                <span style={{color:u2===user?T.green:T.textMid,fontWeight:u2===user?600:400}}>{u2}{u2===user&&' (vos)'}</span>
                <div style={{display:'flex',gap:8,alignItems:'center'}}><span style={{fontFamily:'Bebas Neue',fontSize:15,color:T.textPri,letterSpacing:1}}>{mp.h} - {mp.a}</span><span style={{...st.chip(p2>0?'green':''),fontSize:10}}>{p2} pts</span></div>
              </div>);})}
            </div>)}
          </div>);
        })}
...
function TableTab({allPreds,mr,gr,fr,appState,currentUser,reloadAllPreds}){
  ...
  return(<div>
    <div style={{display:'flex',justifyContent:'space-between',alignItems:'center',marginBottom:20}}>
      <p style={{fontSize:13,color:T.textSec}}>Puntuación en tiempo real</p>
      <button style={{...st.btn(),fontSize:12,padding:'6px 12px',border:`1px solid ${T.border}`}} onClick={refresh} disabled={loading}>{loading?'...':'↻ Actualizar'}</button>
    </div>
    <Leaderboard data={rows} currentUser={currentUser}/>
    <div style={{marginTop:16,padding:'12px 16px',background:T.bgCardHi,borderRadius:10,border:`1px solid ${T.border}`,fontSize:12,color:T.textSec,lineHeight:1.8}}>
      <span style={{color:T.textMid,fontWeight:600}}>Partidos:</span> Exacto: 5 pts · Ganador/empate: 3 pts<br/>
      <span style={{color:T.textMid,fontWeight:600}}>Al terminar grupos:</span> 1°+2° correcto: 5 pts · Solo 1°: 3 · Solo 2°: 1 · Mejor 3°: 1 c/u<br/>
      <span style={{color:T.textMid,fontWeight:600}}>Al terminar el torneo:</span> Campeón: 10 · Sub: 5 · 3°: 3 · 4°: 1 · Premios FIFA: hasta 5 c/u
    </div>
  </div>);
}
...
Se cargaron nuevos resultados. Entrá al prode para ver cuántos puntos sumaste y cómo quedó la tabla. ¡A ver quién acertó!`,
  ...
  return (
    <div style={{...st.card, marginTop:16}}>
      <div style={{fontSize:13,fontWeight:600,color:T.textPri,marginBottom:8}}>📲 Mensajes grupales por WhatsApp</div>
      <p style={{fontSize:12,color:T.textSec,marginBottom:12}}>Enviá mensajes prediseñados al grupo de WhatsApp.</p>
      <div style={{display:'flex',flexDirection:'column',gap:8}}>
        {messages.map((m,i)=>(
          <button key={i}
            onClick={()=>sendWA(m.text())}
            style={{...st.btn(),border:`1px solid ${T.border}`,padding:'10px 14px',display:'flex',alignItems:'center',gap:10,textAlign:'left',fontSize:12,color:T.textPri}}>
...
function InvitePanel({group}) {
  ...
  return (
    <div>
      <p style={{ fontSize: 13, color: T.textSec, marginBottom: 16 }}>
        Generá un código y compartilo. El invitado lo ingresa al registrarse y queda en el grupo automáticamente.
      </p>

      {/* Instrucciones */}
      <div style={{ ...st.card, background: T.bgCardHi, marginBottom: 16, padding: '14px 16px' }}>
        <div style={{ fontSize: 12, color: T.textMid, lineHeight: 1.8 }}>
          <div>1. Generá un código de invitación abajo</div>
          <div>2. Compartilo por WhatsApp, mail o donde quieras</div>
          <div>3. El invitado abre el prode → "<strong style={{ color: T.textPri }}>Tengo un código</strong>" → ingresa el código → se registra → queda en el grupo</div>
        </div>
      </div>
      ...
      {codes.length > 0 && (
        <div style={{ marginBottom: 16 }}>
          <div style={{ fontSize: 12, fontWeight: 600, letterSpacing: '0.08em', color: T.textSec, textTransform: 'uppercase', marginBottom: 10 }}>
            Códigos activos
          </div>
          {codes.map(({ code, created }) => (
            <div key={code} style={{ ...st.card, display: 'flex', alignItems: 'center', gap: 12, padding: '12px 16px', marginBottom: 8 }}>
              <div style={{ flex: 1 }}>
                <div style={{ fontFamily: 'Bebas Neue', fontSize: 26, letterSpacing: 4, color: T.green }}>{code}</div>
                <div style={{ fontSize: 11, color: T.textSec, marginTop: 2 }}>
                  Creado {new Date(created).toLocaleDateString('es-AR')}
                </div>
              </div>
              <div style={{ display: 'flex', gap: 8, flexWrap: 'wrap' }}>
                <button
                  ...
                  style={{ ...st.btn('p'), padding: '7px 14px', fontSize: 12, display: 'flex', alignItems: 'center', gap: 6 }}
                >
                  <span>📲</span> Enviar por WhatsApp
                </button>
                <button
                  onClick={() => copyCode(code)}
                  style={{ ...st.btn('g'), padding: '7px 14px', fontSize: 12, border: `1px solid ${T.green}44` }}
                >
                  {copied === code ? '✓ Copiado' : 'Copiar código'}
                </button>
                <button
                  onClick={() => deleteCode(code)}
                  style={{ ...st.btn('x'), padding: '7px 14px', fontSize: 12, border: `1px solid ${T.red}44` }}
                >
                  Eliminar
                </button>
              </div>
      ...
      <button
        style={{ ...st.btn('p'), padding: '10px 24px', opacity: generating ? 0.7 : 1 }}
...
Respondé SOLO con el JSON, sin texto adicional.`;
  ...
  return (
    <div style={{...st.card,marginTop:8,borderColor:`rgba(116,172,223,0.3)`}}>
      <div style={{fontSize:13,fontWeight:600,color:T.textPri,marginBottom:8}}>🌐 Sincronizar con FIFA (resultados en vivo)</div>
      <p style={{fontSize:12,color:T.textSec,marginBottom:12}}>
        Usa inteligencia artificial para buscar y cargar automáticamente los últimos resultados del Mundial. Revisá los cambios antes de guardar.
      </p>
      <button
        style={{...st.btn('p'),fontSize:12,opacity:syncing?0.7:1}}
        onClick={syncResults}
        disabled={syncing}
      >
        {syncing ? '⏳ Sincronizando...' : '🔄 Obtener resultados en vivo'}
      </button>
      {syncLog && (
        <div style={{marginTop:10,padding:'8px 12px',background:syncLog.startsWith('✓')?T.greenDim:T.redDim||'rgba(244,63,94,0.1)',borderRadius:8,fontSize:12,color:syncLog.startsWith('✓')?T.green:T.red||'#f43f5e',border:`1px solid ${syncLog.startsWith('✓')?T.green+'33':'rgba(244,63,94,0.3)'}`}}>
          {syncLog}
        </div>
      )}
    </div>
...
function AdminTab({mr,gr,fr,appState,onSave,group,setGroup,user}){
  ...
  const[msg,setMsg]=useState('');const[sec,setSec]=useState('matches');const[toRm,setToRm]=useState('');
  const save=()=>{onSave(mrL,grL,frL,stL);setMsg('✓ Guardado');setTimeout(()=>setMsg(''),3000);};
  const removeMember=()=>{if(!toRm||toRm===user)return;const gs=DB['p26_groups']||{};const k=group.name;if(gs[k]){gs[k].members=gs[k].members.filter(m=>m!==toRm);persist('p26_groups',gs);setGroup({...group,members:gs[k].members});setToRm('');setMsg('✓ Eliminado');setTimeout(()=>setMsg(''),3000);}};
  ...
  return(<div>
    <div style={{display:'flex',gap:6,marginBottom:20,flexWrap:'wrap'}}>
      {aSecs.map(s=><button key={s.k} onClick={()=>setSec(s.k)} style={{padding:'6px 14px',border:`1px solid ${sec===s.k?T.green:T.border}`,borderRadius:20,fontSize:12,cursor:'pointer',background:sec===s.k?T.greenDim:'transparent',color:sec===s.k?T.green:T.textSec,fontFamily:'DM Sans',fontWeight:sec===s.k?600:400}}>{s.l}</button>)}
    </div>

    {sec==='estado'&&(<div>
      <p style={{fontSize:13,color:T.textSec,marginBottom:16}}>Controla qué predicciones están activas y cuándo se calculan puntos extra</p>
      {[{k:'groupsFinished',l:'Fase de grupos finalizada',desc:'Habilita premios FIFA · suma puntos de clasificados y mejores 3° · auto-puebla octavos de final'},{k:'tournamentFinished',l:'Torneo finalizado',desc:'Suma puntos de podio y premios FIFA'}].map(({k,l,desc})=>(
        <div key={k} style={{...st.card,display:'flex',alignItems:'center',gap:16,marginBottom:8}}>
          <div style={{flex:1}}><div style={{fontSize:13,fontWeight:600,color:T.textPri,marginBottom:2}}>{l}</div><div style={{fontSize:12,color:T.textSec}}>{desc}</div></div>
          <div onClick={()=>{
            const newVal=!stL[k];
            setStL(s=>({...s,[k]:newVal}));
            ...
            if(k==='groupsFinished'&&newVal){
              ...
              if(Object.keys(updates).length>0) setMrL(prev=>({...prev,...updates}));
            }
          }} style={{width:44,height:24,borderRadius:12,background:stL[k]?T.green:T.border,position:'relative',cursor:'pointer',transition:'background 0.2s',flexShrink:0}}>
            <div style={{position:'absolute',top:3,left:stL[k]?22:3,width:18,height:18,borderRadius:9,background:'#fff',transition:'left 0.2s',boxShadow:'0 1px 3px rgba(0,0,0,0.3)'}}/>
          </div>
        </div>
      ))}

      {/* Botón auto-poblar manual */}
      <div style={{...st.card,marginTop:8}}>
        <div style={{fontSize:13,fontWeight:600,color:T.textPri,marginBottom:8}}>🔄 Auto-poblar eliminatoria</div>
        <p style={{fontSize:12,color:T.textSec,marginBottom:12}}>Calcula automáticamente los equipos de cada ronda eliminatoria según los resultados de grupos cargados y el bracket oficial de la FIFA.</p>
        <button style={{...st.btn('g'),border:`1px solid ${T.green}44`,fontSize:12}} onClick={()=>{
          const updates=autoPopulateKnockout(grL,mrL);
          if(Object.keys(updates).length>0){setMrL(prev=>({...prev,...updates}));setMsg(`✓ ${Object.keys(updates).length} partidos actualizados`);}
          else setMsg('⚠ Sin datos suficientes — cargá resultados de grupos primero');
          setTimeout(()=>setMsg(''),3000);
        }}>Calcular equipos de eliminatoria</button>
      </div>

      {/* Sincronizar con FIFA via Claude API */}
      <SyncPanel mrL={mrL} setMrL={setMrL} grL={grL} setGrL={setGrL} setMsg={setMsg}/>
    </div>)}

    {sec==='sim'&&(<div>
      <p style={{fontSize:13,color:T.textSec,marginBottom:16}}>Simulá cualquier momento del torneo para testear predicciones y puntajes</p>
      <div style={st.card}>
        <div style={{fontSize:13,fontWeight:600,color:T.textPri,marginBottom:12}}>📅 Fecha y hora simulada (hora Argentina)</div>
        <div style={{display:'flex',gap:10,alignItems:'flex-end',flexWrap:'wrap'}}>
          <div style={{flex:1,minWidth:200}}>
            <input type="datetime-local" style={{...st.inp,marginBottom:0}} value={stL.simDate?new Date(new Date(stL.simDate).getTime()).toISOString().slice(0,16):''} onChange={e=>{if(!e.target.value){setStL(s=>({...s,simDate:null}));return;}const localMs=new Date(e.target.value).getTime();const artOffset=-3*60*60*1000;const utcMs=localMs-artOffset-new Date().getTimezoneOffset()*60000;setStL(s=>({...s,simDate:new Date(utcMs).toISOString()}));}}/>
          </div>
          <button style={{...st.btn('x'),marginBottom:8,border:`1px solid ${T.red}44`}} onClick={()=>setStL(s=>({...s,simDate:null}))}>Limpiar</button>
        </div>
        {stL.simDate&&<div style={{marginTop:10,padding:'8px 12px',background:T.greenDim,borderRadius:8,fontSize:12,color:T.green,border:`1px solid ${T.green}33`}}>⚠ Simulando: <strong>{new Date(stL.simDate).toLocaleString('es-AR',{timeZone:'America/Argentina/Buenos_Aires',dateStyle:'full',timeStyle:'short'})}</strong></div>}
        {!stL.simDate&&<div style={{marginTop:10,padding:'8px 12px',background:T.bgCardHi,borderRadius:8,fontSize:12,color:T.textSec}}>Usando hora real · {new Date().toLocaleString('es-AR',{timeZone:'America/Argentina/Buenos_Aires',dateStyle:'full',timeStyle:'short'})}</div>}
      </div>
      <div style={st.card}>
        <div style={{fontSize:13,fontWeight:600,color:T.textPri,marginBottom:12}}>⚡ Atajos</div>
        <div style={{display:'flex',flexWrap:'wrap',gap:8}}>
          {[{l:'Inicio del torneo',d:'2026-06-11T18:00:00Z'},{l:'Jornada 1 finalizada',d:'2026-06-14T06:00:00Z'},{l:'Jornada 2 finalizada',d:'2026-06-20T06:00:00Z'},{l:'Grupos finalizados',d:'2026-06-26T06:00:00Z'},{l:'Final del mundial',d:'2026-07-19T23:00:00Z'}].map(({l,d})=>(
            <button key={l} style={{...st.btn(),fontSize:12,padding:'7px 14px',border:`1px solid ${T.border}`}} onClick={()=>setStL(s=>({...s,simDate:d}))}>{l}</button>
          ))}
        </div>
      </div>
      <div style={st.card}>
        <div style={{fontSize:13,fontWeight:600,color:T.textPri,marginBottom:10}}>Estado de partidos con fecha simulada</div>
        {MATCHES.map(m=>{const simNow=stL.simDate?new Date(stL.simDate):new Date();const lk=new Date(m.kickoff.getTime()-30*60*1000);const op=new Date(m.kickoff);op.setUTCHours(3,0,0,0);if(op>m.kickoff)op.setUTCDate(op.getUTCDate()-1);const r=mrL[m.id]||{};let st='upcoming';if(r.played)st='played';else if(r.locked||simNow>=lk)st='locked';else if(simNow>=op)st='open';const stColors={upcoming:T.textSec,open:T.green,locked:T.gold,played:T.green};const stLabels={upcoming:'⏳',open:'🟢',locked:'🔒',played:'✓'};return(<div key={m.id} style={{display:'flex',justifyContent:'space-between',alignItems:'center',padding:'7px 0',borderBottom:`1px solid ${T.border}22`,fontSize:12}}><span style={{color:T.textMid}}>{m.dateART} {m.timeART} · <strong style={{color:T.textPri}}>{m.h} vs {m.a}</strong></span><span style={{color:stColors[st],fontWeight:600}}>{stLabels[st]} {st}</span></div>);})}
      </div>
    </div>)}

    {sec==='matches'&&(<div>
      <p style={{fontSize:13,color:T.textSec,marginBottom:16}}>Cargá resultados y bloqueá predicciones. Para eliminatoria, primero definí los equipos.</p>
      {MATCHES.map(m=>{const r=mrL[m.id]||{h:'',a:'',played:false,locked:false};const isTBD=m.tbd&&(!r.teamH&&!m.h||!r.teamA&&!m.a);return(<div key={m.id} style={{...st.card,marginBottom:8}}>
        <div style={{fontSize:12,color:m.round>=4?T.gold:T.textSec,marginBottom:8,fontWeight:m.round>=4?600:400}}>{m.round>=4?`${m.label||'Eliminatoria'} (Ronda ${m.round})`:`Grupo ${m.g} · Jornada ${m.round}`}</div>
        <div style={{fontSize:12,color:T.textSec,marginBottom:10,display:'flex',alignItems:'center',gap:6}}>
          {m.round>=4?(<>
            <input placeholder="Equipo local" value={r.teamH||m.h||''} onChange={e=>setMrL(p=>({...p,[m.id]:{...r,teamH:e.target.value}}))} style={{flex:1,padding:'5px 8px',border:`1px solid ${T.border}`,borderRadius:6,background:T.bgInput,color:T.textPri,fontSize:12}}/>
            <span style={{color:T.textSec}}>vs</span>
            <input placeholder="Equipo visitante" value={r.teamA||m.a||''} onChange={e=>setMrL(p=>({...p,[m.id]:{...r,teamA:e.target.value}}))} style={{flex:1,padding:'5px 8px',border:`1px solid ${T.border}`,borderRadius:6,background:T.bgInput,color:T.textPri,fontSize:12}}/>
          </>):(<><Team name={m.h} size={12}/><span style={{color:T.textSec,marginInline:4}}>vs</span><Team name={m.a} size={12}/></>)}
        </div>
        <div style={{display:'flex',gap:10,alignItems:'center',flexWrap:'wrap'}}>
          <div style={{display:'flex',gap:6,alignItems:'center'}}>
            <input type="number" min={0} max={20} value={r.h??''} onChange={e=>setMrL(p=>({...p,[m.id]:{...r,h:+e.target.value}}))} style={{width:52,height:40,textAlign:'center',border:`1px solid ${T.border}`,borderRadius:8,background:T.bgInput,color:T.textPri,fontSize:18,fontFamily:'Bebas Neue',outline:'none'}}/>
            <span style={{color:T.textSec,fontFamily:'Bebas Neue',fontSize:16}}>-</span>
            <input type="number" min={0} max={20} value={r.a??''} onChange={e=>setMrL(p=>({...p,[m.id]:{...r,a:+e.target.value}}))} style={{width:52,height:40,textAlign:'center',border:`1px solid ${T.border}`,borderRadius:8,background:T.bgInput,color:T.textPri,fontSize:18,fontFamily:'Bebas Neue',outline:'none'}}/>
          </div>
          <label style={{display:'flex',gap:6,alignItems:'center',fontSize:12,cursor:'pointer',color:T.textMid}}><input type="checkbox" checked={!!r.played} onChange={e=>setMrL(p=>({...p,[m.id]:{...r,played:e.target.checked}}))}/>Jugado</label>
          <label style={{display:'flex',gap:6,alignItems:'center',fontSize:12,cursor:'pointer',color:T.textMid}}><input type="checkbox" checked={!!r.locked} onChange={e=>setMrL(p=>({...p,[m.id]:{...r,locked:e.target.checked}}))}/>Bloqueado</label>
        </div>
      </div>);})}
    </div>)}

    {sec==='groups'&&(<div>
      <p style={{fontSize:13,color:T.textSec,marginBottom:16}}>Clasificados reales por grupo</p>
      <div style={{display:'grid',gridTemplateColumns:'repeat(auto-fill,minmax(195px,1fr))',gap:8,marginBottom:12}}>
        {GRP.map(gg=>{const g2=grL.groups?.[gg.id]||{};return(<div key={gg.id} style={{...st.card,padding:'12px 14px'}}>
          <div style={{fontFamily:'Bebas Neue',fontSize:14,letterSpacing:2,color:T.green,marginBottom:10}}>GRUPO {gg.id}</div>
          <select value={g2.first||''} onChange={e=>setGrL(p=>({...p,groups:{...p.groups,[gg.id]:{...g2,first:e.target.value}}}))} style={{...st.inp,marginBottom:6}}><option value=''>1° lugar...</option>{gg.t.map(t=><option key={t} value={t}>{t}</option>)}</select>
          <select value={g2.second||''} onChange={e=>setGrL(p=>({...p,groups:{...p.groups,[gg.id]:{...g2,second:e.target.value}}}))} style={{...st.inp,marginBottom:0}}><option value=''>2° lugar...</option>{gg.t.map(t=><option key={t} value={t}>{t}</option>)}</select>
        </div>);})}
      </div>
      <div style={st.card}>
        <div style={{fontSize:13,fontWeight:600,color:T.textPri,marginBottom:10}}>Mejores terceros clasificados · <span style={{color:T.green}}>{(grL.thirds||[]).length}/8</span></div>
        <div style={{display:'flex',flexWrap:'wrap',gap:6}}>
          {ALL_TEAMS.map(t=>{const s=(grL.thirds||[]).includes(t);return(<button key={t} onClick={()=>{const cur=grL.thirds||[];setGrL(p=>({...p,thirds:s?cur.filter(x=>x!==t):cur.length<8?[...cur,t]:cur}));}} style={{display:'inline-flex',alignItems:'center',gap:5,padding:'5px 10px',border:`1px solid ${s?T.green:T.border}`,borderRadius:20,fontSize:11,cursor:'pointer',background:s?T.greenDim:'transparent',color:s?T.green:T.textSec,transition:'all 0.1s'}}><Flag name={t} h={13}/>{t}</button>);})}
        </div>
      </div>
    </div>)}

    {sec==='podium'&&(<div>
      <p style={{fontSize:13,color:T.textSec,marginBottom:16}}>Podio real del torneo</p>
      <div style={{display:'grid',gridTemplateColumns:'repeat(2,1fr)',gap:10}}>
        {[{k:'p1',l:'🥇 Campeón'},{k:'p2',l:'🥈 Subcampeón'},{k:'p3',l:'🥉 3° Puesto'},{k:'p4',l:'4° Puesto'}].map(({k,l})=>(<div key={k} style={st.card}><div style={{fontSize:13,fontWeight:600,color:T.textPri,marginBottom:8}}>{l}</div><select value={frL.podium?.[k]||''} onChange={e=>setFrL(p=>({...p,podium:{...p.podium,[k]:e.target.value}}))} style={{...st.inp,marginBottom:0}}><option value=''>Elegir...</option>{ALL_TEAMS.map(t=><option key={t} value={t}>{t}</option>)}</select></div>))}
      </div>
    </div>)}

    {sec==='awards'&&(<div>
      <p style={{fontSize:13,color:T.textSec,marginBottom:16}}>Ganadores reales de premios FIFA</p>
      <div style={{display:'grid',gridTemplateColumns:'repeat(auto-fill,minmax(210px,1fr))',gap:10}}>
        {AWARDS.map(aw=>(<div key={aw.id} style={st.card}><div style={{fontSize:13,fontWeight:600,color:T.textPri,marginBottom:8}}>{aw.label}</div><select style={{...st.inp,marginBottom:0}} value={frL.awards?.[aw.id]||''} onChange={e=>setFrL(p=>({...p,awards:{...p.awards,[aw.id]:e.target.value}}))}>
    ...
    {sec==='members'&&(<div>
      <p style={{fontSize:13,color:T.textSec,marginBottom:16}}>Participantes del grupo</p>
      {group.members.map(m=>(<div key={m} style={{...st.card,display:'flex',justifyContent:'space-between',alignItems:'center',padding:'12px 16px',marginBottom:6}}>
        <div style={{display:'flex',alignItems:'center',gap:10}}>
          <div style={{width:32,height:32,borderRadius:8,background:T.bgCardHi,border:`1px solid ${T.border}`,display:'flex',alignItems:'center',justifyContent:'center',fontSize:13,color:T.textMid,fontWeight:600}}>{m[0].toUpperCase()}</div>
          <span style={{fontSize:13,color:T.textPri,textTransform:'capitalize'}}>{m}{m===user&&<span style={{color:T.textSec}}> (vos)</span>}{m===group.admin&&<span style={{color:T.gold}}> 👑</span>}</span>
        </div>
        {m!==user&&m!==group.admin&&<button style={{...st.btn('x'),padding:'5px 12px',fontSize:11,border:`1px solid ${T.red}44`}} onClick={()=>setToRm(m)}>Eliminar</button>}
      </div>))}
      {toRm&&(<div style={{...st.card,marginTop:8,borderColor:`${T.red}44`}}><p style={{fontSize:13,color:T.textPri,marginBottom:12}}>¿Eliminar a <strong>{toRm}</strong> del grupo?</p><div style={{display:'flex',gap:8}}><button style={{...st.btn('x'),border:`1px solid ${T.red}44`}} onClick={removeMember}>Confirmar</button><button style={{...st.btn(),border:`1px solid ${T.border}`}} onClick={()=>setToRm('')}>Cancelar</button></div></div>)}
    </div>)}

    {sec==='invites'&&<InvitePanel group={group}/>}
    {sec==='invites'&&<AdminWhatsApp group={group} mr={mrL} appState={stL}/>}

    <div style={{marginTop:24,paddingTop:16,borderTop:`1px solid ${T.border}`,display:'flex',gap:12,alignItems:'center'}}>
      <button style={st.btn('p')} onClick={save}>Guardar cambios</button>
      {msg&&<span style={{fontSize:13,color:msg.startsWith('✓')?T.green:T.red,fontWeight:500}}>{msg}</span>}
    </div>
  </div>);
}
...
function Leaderboard({data,currentUser}){
  if(!data.length)return(<div style={{textAlign:'center',padding:'40px 20px',color:T.textSec}}><p style={{fontSize:14}}>Sin datos. Los puntos aparecen cuando hay resultados cargados.</p></div>);
  const medals=['🥇','🥈','🥉'];
  return(<div style={{display:'flex',flexDirection:'column',gap:6}}>
    {data.map((row,i)=>{const isMe=row.user===currentUser;return(
      <div key={row.user} style={{display:'flex',alignItems:'center',gap:12,padding:'12px 16px',borderRadius:12,border:`1px solid ${isMe?T.green:T.border}`,background:isMe?T.greenDim:T.bgCard,transition:'all 0.15s'}}>
        <div style={{fontFamily:'Bebas Neue',fontSize:20,minWidth:32,textAlign:'center',color:i===0?T.gold:i===1?'#9ca3af':i===2?'#cd7f32':T.textSec}}>{medals[i]||`${i+1}`}</div>
        <div style={{flex:1}}>
          <span style={{fontWeight:isMe?600:500,fontSize:14,color:isMe?T.green:T.textPri,textTransform:'capitalize'}}>{row.user}</span>
          {isMe&&<span style={{fontSize:11,color:T.green,marginLeft:6,opacity:0.8}}>· vos</span>}
          {row.exactos>0&&<span style={{fontSize:10,color:T.textSec,marginLeft:6}}>{row.exactos} exacto{row.exactos!==1?'s':''}</span>}
        </div>
        <div style={{fontFamily:'Bebas Neue',fontSize:26,color:isMe?T.green:T.textPri,letterSpacing:1}}>{row.score}<span style={{fontSize:13,color:T.textSec,fontFamily:'DM Sans',fontWeight:400,letterSpacing:0,marginLeft:3}}>pts</span></div>
      </div>
    );})}
  </div>);
}
